...
```{sql, connection="mydb"}
INSERT INTO nonsense_df_1 (row_names, id_student, nonsense_1, nonsense_2, nonsense_3)
VALUES (0, 1, 2, 3, 4); 
```
...
```{sql, connection="mydb"}
SELECT SUM(row_names)
FROM nonsensedf1;
```

```{sql, connection="mydb"}
SELECT first.row_names, second.row_names, first.id_student, second.id_student
FROM nonsensedf1 AS first
RIGHT JOIN nonsense_df_2 AS second
ON first.row_names = second.row_names
WHERE first.id_student = NULL
LIMIT 100;
```