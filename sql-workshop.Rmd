---
title: "sql-workshop Response"
author: "Timothy Lee"
date: "29/01/2020"
output: html_document
---

# Connecting to and populating the AWS database

## Connect to AMW MySQL Database

*Database connection details have been saved in a .RData file that has been added to .gitignore. Details available on request*
...
*Data used here have been added to .gitignore. Data can be downloaded from https://analyse.kmi.open.ac.uk/open_dataset*

```{r}
#Student demographic data
studentInfo <- read.csv("anonymisedData/studentInfo.csv", header = TRUE)
#Student assessment data
studentAssessment <- read.csv("anonymisedData/studentAssessment.csv", header = TRUE)
#Course data
courses <- read.csv("anonymisedData/courses.csv", header = TRUE)
studentRegistration <- read.csv("anonymisedData/studentRegistration.csv", header = TRUE)
```
...
nonsense_1 <- sample(1:10000, size = 32593, replace = TRUE)
nonsense_2 <- sample(10001:20000, size = 32593, replace = TRUE)
nonsense_3 <- sample(20001:30000, size = 32593, replace = TRUE)
nonsense_4 <- sample(30001:40000, size = 32593, replace = TRUE)
nonsense_5 <- sample(40001:50000, size = 32593, replace = TRUE)
nonsense_6 <- sample(50001:60000, size = 32593, replace = TRUE)

nonsense_df_1 <- data.frame(id_student = studentInfo$id_student, nonsense_1, nonsense_2, nonsense_3)
nonsense_df_2 <- data.frame(id_student = studentInfo$id_student, nonsense_4, nonsense_5, nonsense_6)

dbWriteTable(mydb, "nonsense_df_1", nonsense_df_1)
dbWriteTable(mydb, "nonsense_df_2", nonsense_df_2)
...
SELECT id_student AS 'Student ID', nonsense_1 AS 'Nonsense'
FROM nonsense_df_1
ORDER BY nonsense_1 DESC
LIMIT 20;

```
```{sql, connection="mydb"}
SELECT id_student AS 'Student ID', nonsense_5 AS 'Nonsense'
FROM nonsense_df_2
WHERE nonsense_5 > 45000
ORDER BY id_student
LIMIT 20;
...
```{sql, connection="mydb"}
INSERT INTO nonsense_df_1 (row_names, nonsense_1, nonsense_2, nonsense_3)
VALUES (0, 2, 3, 4);
```
...
Putting back the edited row that was deleted, so it returns to how it was from the start.

```{sql, connection="mydb"}
INSERT INTO nonsense_df_2 (row_names, id_student, nonsense_4, nonsense_5, nonsense_6)
VALUES (1, 11391, 34148, 45793, 55204);
```
...
#Inserting a NULL row - this produces an error
...
```{sql, connection="mydb"}
DROP TABLE nonsensedf1;
#Dropping the new table instead of the 'original' (old) table - this maintains naming consistency between the two toy tables
```
...
```{sql, connection="mydb"}
CREATE TABLE nonsensedf2 (
row_names TEXT,
id_student INTEGER,
nonsense_4 INTEGER DEFAULT 0,
nonsense_5 INTEGER,
nonsense_6 INTEGER
);
```
...
```{sql, connection="mydb"}
DROP TABLE IF EXISTS nonsensedf2;
```
...
```{sql, connection="mydb"}
ALTER TABLE nonsensedf1 ADD nonsense_7 INTEGER DEFAULT 3;
...
```
```{sql, connection="mydb"}
ALTER TABLE nonsensedf1 DROP nonsense_7;
...
```{sql, connection="mydb"}
INSERT INTO exercise7 (var1) VALUES ('lol');
...
SELECT *
FROM nonsensedf1
WHERE nonsense_1 > 5000 AND nonsense_2 < 12000;
...
SELECT *
FROM nonsensedf1
WHERE row_names IN ('5','8','15000','28');
...
```{sql, connection="mydb"}
INSERT INTO nonsensedf1 (row_names, id_student, nonsense_1, nonsense_2, nonsense_3)
VALUES (2000, 550381, 1517, 16882, 24415);
```

```{sql, connection="mydb"}
SELECT row_names, id_student
FROM nonsensedf1
WHERE row_names LIKE '200_'
ORDER BY row_names
LIMIT 5;
...
```{sql, connection="mydb"}
SELECT DISTINCT row_names, id_student
FROM nonsensedf1
WHERE row_names LIKE '200_'
ORDER BY row_names
LIMIT 5;
...
dbGetQuery(mydb,"SELECT studentInfo.id_student, gender, region, date_registration
          FROM studentInfo
...
## EXERCISE 10
Create a common id variable in your two toy data tables. Then do join those tables so that your query returns all the values from one table and only those that match from the other.

```{sql, connection="mydb"}
UPDATE nonsense_df_2
SET row_names = row_names - 10000;
```

```{sql, connection="mydb"}
SELECT row_names
FROM nonsense_df_2
ORDER BY row_names DESC
LIMIT 5;
```
...
```{sql, connection="mydb"}
SELECT first.row_names AS a, second.row_names, first.id_student, second.id_student
FROM nonsensedf1 AS first
LEFT JOIN nonsense_df_2 AS second
ON first.row_names = second.row_names
LIMIT 10;
```
